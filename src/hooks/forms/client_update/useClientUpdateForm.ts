...
const useClientUpdateForm = (
  { id, firstname, lastname, phoneNumber, ifuNumber, email }: FormData,
  modalLabel: string
) => {
  ...
  const onFormSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      if (response!.status == 406) {
        ...
        fetchAllClients();
        setActionResultMessage("Le client a été modifié avec succès");
...
export default useClientUpdateForm;
